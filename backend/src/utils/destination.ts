...
export async function resolveDestinationInput(params: {
...
}): Promise<{ destinationId: string | null; destination: string | null; found: boolean }> {
  if (params.destinationId) {
    const byId = await prisma.destination.findUnique({
      where: { id: params.destinationId },
      select: { id: true, name: true },
    });
    if (byId) return { destinationId: byId.id, destination: byId.name, found: true };
  }
  ...
  if (params.destination) {
    ...
    if (byName) return { destinationId: byName.id, destination: byName.name, found: true };
  }
