...
clientsRouter.post('/', async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: 'Invalid client payload' });
  ...
  const accommodationLevel = body.accommodationLevel ? apiToPrisma(body.accommodationLevel as any) : 'three';
  ...
  const occupancy = (body.occupancy ?? 'double') as any;
...
clientsRouter.put('/:id', async (req, res) => {
  ...
  if (body.accommodationLevel !== undefined) data.accommodationLevel = apiToPrisma(body.accommodationLevel as any);
