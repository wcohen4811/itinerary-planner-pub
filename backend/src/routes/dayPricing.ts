...
dayPricingRouter.get('/:itineraryId/pricing/levels/:level', async (req, res) => {
  const { itineraryId, level } = req.params as { itineraryId: string; level: '3' | '4' | '5' | 'deluxe' };
...
dayPricingRouter.put('/:itineraryId/pricing/levels/:level/days/:dayId', async (req, res) => {
  const { itineraryId, level, dayId } = req.params as { itineraryId: string; dayId: string; level: '3' | '4' | '5' | 'deluxe' };
  const it = await prisma.itinerary.findFirst({ where: { id: itineraryId } });
  if (!it) return res.status(404).json({ error: 'Itinerary not found' });
  const lv = levelApiToDb(level);
  const d = req.body ?? {};
