...
function base64Bytes(raw: string) {
...
itinerariesRouter.get('/library/days', async (req, res) => {
  ...
  const where: Prisma.DayWhereInput | undefined = q
    ? {
        OR: [
...
itinerariesRouter.put('/:id', async (req, res) => {
  ...
  if (blankPricing !== undefined) data.blankPricing = !!blankPricing;
