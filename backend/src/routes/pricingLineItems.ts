...
pricingLineItemsRouter.get('/:itineraryId/pricing/line-items', async (req, res) => {
  const { itineraryId } = req.params as { itineraryId: string };
...
pricingLineItemsRouter.post('/:itineraryId/pricing/line-items', async (req, res) => {
  ...
  const lv = apiToPrisma(level ?? prismaToApi(it.accommodationLevel as any));
  const template =
    body.templateId ? await prisma.pricingLineItemTemplate.findUnique({ where: { id: body.templateId } }) : null;
...
pricingLineItemsRouter.put('/:itineraryId/pricing/line-items/:itemId', async (req, res) => {
  const { itineraryId, itemId } = req.params as { itineraryId: string; itemId: string };
  ...
  if (body.amountUsd !== undefined) data.amountUsd = Math.max(0, Math.floor(Number(body.amountUsd)));
...
pricingLineItemsRouter.post('/:itineraryId/days/:dayId/pricing/items', async (req, res) => {
  const { itineraryId, dayId } = req.params as { itineraryId: string; dayId: string };
  ...
  const level = body.accommodationLevel as '3' | '4' | '5' | 'deluxe';
  const occupancy = parseOccupancy(body.occupancy);
  const lv = apiToPrisma(level ?? prismaToApi(day.accommodationLevel as any));
  const template =
    body.templateId ? await prisma.pricingLineItemTemplate.findUnique({ where: { id: body.templateId } }) : null;
  const templatePrice = template
    ? await prisma.pricingLineItemTemplatePrice.findUnique({
...
pricingLineItemsRouter.put('/:itineraryId/days/:dayId/pricing/items/:itemId', async (req, res) => {
  const { itineraryId, dayId, itemId } = req.params as { itineraryId: string; dayId: string; itemId: string };
  const item = await prisma.dayPricingItem.findFirst({
    where: { id: itemId, dayId },
    include: { day: true },
  });
  if (!item || item.day.itineraryId !== itineraryId) return res.status(404).json({ error: 'Line item not found' });
