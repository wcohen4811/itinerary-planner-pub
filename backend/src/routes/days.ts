...
daysRouter.post('/', (req, res) => {
  ...
  const pricing = computeDayPricing({
    ...
    transferCount: (base as any).transferCount ?? 0,
    destination: (base as any).destination ?? (base as any).provider,
...
daysRouter.post('/batch', (req, res) => {
  ...
  const created = dayStore.createMany(
    parse.data.days.map((base) => ({
      ...base,
...
daysRouter.put('/:id', (req, res) => {
  ...
  const pricing = computeDayPricing({
    ...
    transferCount: (merged as any).transferCount ?? 0,
    destination: (merged as any).destination ?? (merged as any).provider,
