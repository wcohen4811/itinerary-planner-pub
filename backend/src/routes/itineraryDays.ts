...
itineraryDaysRouter.post('/:itineraryId/days/clone', async (req, res) => {
  ...
  if (!sourceDayId) return res.status(400).json({ error: 'sourceDayId required' });
...
itineraryDaysRouter.put('/:itineraryId/days/:dayId', async (req, res) => {
  ...
  const d = req.body ?? {};
  const data: any = {};
  if (typeof d.dayNumber === 'number') data.dayNumber = d.dayNumber;
  if (d.title) data.title = d.title;
  if (d.description) data.description = d.description;
  if (d.hotelName !== undefined) data.hotelName = d.hotelName;
  if (d.accommodationLevel) data.accommodationLevel = apiToPrisma(d.accommodationLevel);
  ...
  if (d.transferStatus) data.transferStatus = d.transferStatus;
  ...
  if (d.components !== undefined) data.components = d.components;
